  ...
  useEffect(() => {
    const fetchBlogs = async () => {
      try {
        ...
        const filtered = category === "ALL" ? res.data : res.data.filter(b => b.category === category);
        setBlogs(filtered);
      } catch (err) {
  ...
  return (
    <Container maxWidth="lg" sx={{ py: 4 }}>
      ...
      {loading ? (
        ...
        <Typography sx={{ textAlign: "center", mt: 5 }}>No blogs found in this category.</Typography>
      ) : (
